# AoE RTS Engine — Milestone 3: Military Phase ロードマップ（Phase 35〜40）
...
> **前:** M2.6 RTS UX / **次:** [07_M4_GAMEPLAY_PHASES.md](07_M4_GAMEPLAY_PHASES.md)（Phase 41〜45）

> **番号変更:** M2.6（Phase 29〜32）挿入に伴い旧 Phase 29〜34 を **Phase 33〜38** に繰り下げ。  
> **2026-06 追記:** M2.5 に Phase 25（Info Panel）・Phase 28（Sheep Herding）挿入に伴い **Phase 35〜40** へ再繰り下げ。
...
| Phase | 名称 | 目的 | 状態 |
|-------|------|------|------|
| 35 | Archer | 遠距離攻撃・弾丸（または即時ヒット） | ⬜ 未着手 |
| 36 | Spearman | 槍兵・対騎兵ボーナス下地 | ⬜ 未着手 |
| 37 | Cavalry | 騎兵・高機動近接 | ⬜ 未着手 |
| 38 | Counter System | 装甲・ボーナスダメージ相性（Melee/Pierce 拡張） | ⬜ 未着手 |
| 39 | Stance & Aggro | Stand Ground / Defensive / 完全 Aggro | ⬜ 未着手 |
| 40 | Formation | 隊列移動・軽量 Separation | ⬜ 未着手 |

**M3 完了条件:** 歩兵 3 種 + 騎兵 + 相性表 + スタンス + グループ隊列が `Phase10.unity` で動作。

**Phase 29 との関係:** M2.5 で **簡易 Militia Aggro** を先行実装。Phase 39 でスタンス UI と弓兵 Aggro を統合。

**Phase 25 との関係:** M2.5 **Selection Info Panel** で単一 `armor` 表示。Phase 38 で Melee/Pierce 2 種に拡張。
...
3. Phase 35 から順に small diff → `Phase10.unity` Play 確認
