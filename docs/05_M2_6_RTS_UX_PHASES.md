# AoE RTS Engine — Milestone 2.6: RTS UX ロードマップ（Phase 29〜32）
...
> **前:** M2.5 Economy Polish / **次:** [06_M3_MILITARY_PHASES.md](06_M3_MILITARY_PHASES.md)（Phase 33〜）
...
|------|---------------|---------------------|
| **ユニット生産キュー** | TC / Barracks 等で **Villager・Militia 等を複数予約**（Q 連打、Shift+5 体）。資源は **キュー追加時に消費** | **Phase 29** ← 今回 M2.6 の主題 |
| **集合地点（Rally / Gather Point）** | 生産建物を選択 → 右クリックで **生まれたユニットの行き先**（資源・前線） | **Phase 31** |
...
**結論:** ユーザーが言う「Villager / Militia の作成予約」= **ユニット生産キュー（Phase 29）**。建物そのものを並べて予約する機能ではない。
...
| Phase | 名称 | 目的 | 状態 |
|-------|------|------|------|
| 29 | Unit Production Queue | TC / Barracks で複数ユニット予約 + Barracks Q | ⬜ 未着手 |
| 30 | Idle Unit UX | 待機村民・軍の表示 + 選択ホットキー | ⬜ 未着手 |
| 31 | Rally Point | 生産建物の集合地点（右クリック） | ⬜ 未着手 |
| 32 | Control Groups | Ctrl+数字で選択グループ保存 | ⬜ 未着手 |
...
## Phase 29 — Unit Production Queue ⬜
...
3. **資源** — キュー **追加時に即 Spend**（Pop / Food / Wood チェック）。キャンセル時返金は Phase 29 では **任意**（MVP はキャンセル UI なしでも可）
...
| 項目 | 先送り先 |
|------|----------|
| Shift+5 体一括キュー | Phase 29 拡張 or M4 |
| キューから個別キャンセル（クリックで返金） | Phase 29 拡張 |
| 複数建物分散キュー | M3 以降 |
| Shift+連続建築配置 | 建築 UX 別 Phase |
| Minimap / 本格 uGUI | M4 以降 |
| 全待機軍事ユニット一括選択 | AoE2 にも弱い — Phase 30 拡張 |
...
4. M2.6 全完了後 → [06_M3_MILITARY_PHASES.md](06_M3_MILITARY_PHASES.md) Phase 33 へ
