...
>
> **最終更新:** Phase 60 ✅（CPU Difficulty System）。**M6 進行中 — 次: Phase 61 Team & 2v2。**
>
...
| **対戦想定** | **4人 FFA（人間1 + CPU3）** または **1v1 CPU**（`MatchMode` / **M** キー切替・シーン再生成で反映）。**M6 目標: 2v2・大マップ・敵HP/修理/複数建設・Fog**（Phase 61〜66） |
...
**Milestone 6 — 4-Player & World Scale:** ⬜ 進行中（Phase 61〜66 が本体 / 67 は LAN 前 — **次: Phase 61**）
...
| 建築選択（TC / Barracks） | ✅ | |
| CPU ユニット・建築の選択不可 | ✅ | `UnitTeam.Enemy` フィルタ — **Phase 63 で敵選択・HP 表示** |
...
| 配置ゴーストプレビュー | ✅ | 有効/無効色 |
| Villager による建築 | ✅ | 現場移動 → 建築タイマー（**1 人/サイト** — Phase 65 で複数人加速） |
| 建築中断（移動命令） | ✅ | Wood 返金なし |
| 建物修理 | ❌ | Phase 64 予定 |
...
| HP バー（選択時） | ✅ | `UnitHpBarView`（OnGUI）— **自軍のみ** / 敵は Phase 63 |
| 攻撃中色変化 | ✅ | オレンジ系ティント |
...
| **4 人 / 2v2** | 1H+3CPU / 同盟 | △ 4人 ✅ | M6 Phase 61 |
| **CPU 難易度** | Easy〜Hardest | ✅ Phase 60 | — |
| **敵 HP 表示** | 敵選択・識別 | ❌（敵選択不可） | M6 Phase 63 |
| **建物修理** | Villager 修理 | ❌ | M6 Phase 64 |
| **複数人建設** | 建築速度加速 | ❌（1人/サイト） | M6 Phase 65 |
| **Fog of War** | 視界制限 | ❌ | M6 Phase 66 |
| **大型マップ** | 4 隅スポーン | △ Phase10 小 | M6 Phase 62 |
...
| CPU 難易度・戦略バリエーション | ✅ Phase 60 — Easy/Normal/Hard/Hardest + 遅延キュー |
...
| 敵 HP 表示 | Phase 63 — 敵選択 + HP バー |
| 建物修理 | Phase 64 — `RepairManager` + 木材コスト |
| 複数 Villager 建設 | Phase 65 — `ConstructionSite` 複数 builder |
...
| P3 | CPU Difficulty | ✅ M6 Phase 60 |
| P3 | 2v2 同盟 | M6 Phase 61 — **次** |
| P4 | 大型マップ | M6 Phase 62 |
| P4 | 敵 HP 表示 | M6 Phase 63 |
| P4 | 建物修理 | M6 Phase 64 |
| P4 | 複数 Villager 建設 | M6 Phase 65 |
| P4 | Fog of War | M6 Phase 66 |
| P5 | 決定論（LAN 前） | M6 Phase 67 |
...
| 次に何を作るべき？ | **Phase 61 Team & 2v2** — [10_M6](10_M6_MULTIPLAYER_FOUNDATION.md) |
