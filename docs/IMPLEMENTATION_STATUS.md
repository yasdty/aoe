...
>
> **最終更新:** Phase 18 完了（Farm 建築 + 採集）。Foundation Milestone 1 完了。M2 Economy 進行中。
>
...
| Command Queue | ✅ | プレイヤー操作 9 種（GatherFood / GatherFarmFood 含む）。`CommandLog` 記録 |
...
| **主要クラス** | `BuildingPlacementManager`, `ProductionManager`, `BarracksProductionManager`, `TownCenter`, `House`, `Barracks`, `Farm`, `RuntimeBuildingFactory`, `PlacedBuildingDataResolver` |
| **関連データ** | `BuildingData`（TC）, `PlacedBuildingData`（House, Barracks, Farm） |
...
| **PlacedBuildingData** | `HouseData`, `BarracksData`, `FarmData` | 配置建築のコスト・建築時間・フットプリント・訓練ユニット・Farm foodCapacity |
...
| -------- | ---------- | --------------------------- |
| **Economy** | **30%** | Wood / Food 採集・TC 搬入 ✅。Gold/Stone・市場・交易 ❌ |
| **Buildings** | **12%** | TC / House / Barracks / Farm の 4 種 ✅（§7）。時代建築・防衛・生産建築多数 ❌ |
...
| 次に何を作るべき？ | **Phase 19 Lumber Camp**（Milestone 2 Economy）— [docs/README.md](README.md) |
