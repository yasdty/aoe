...
| 25 | Selection Info Panel | 左クリック選択時の詳細 + 資源残量表示 | ⬜ 未着手 |
...
- `SelectionInfoPanelView`（OnGUI、左下 — 既存 `ResourceHudView` / `UnitHpBarView` と干渉しない配置）
- **左クリック** でユニット / 建物 / **資源ノード** を単体選択（既存矩形・複数選択は維持）
- 表示内容（MVP）:

| 対象 | 表示 |
|------|------|
| Villager | 名前、HP（`current/max`）— 攻撃力 **非表示**（`attack=0`） |
| Militia | 名前、HP、攻撃力（`UnitData.attack`） |
| Armor | `UnitData.armor` / `BuildingHealth.Armor` — **0 なら非表示** |
| TC / House / Barracks / Camp / Farm | 名前、HP |
| Tree | `Wood: N` 残量 |
| Berry / Deer / Sheep | `Food: N` 残量 |
| Gold / Stone Mine | `Gold: N` / `Stone: N` 残量 |

**AoE2 参考:** Melee / Pierce 装甲の 2 種は **MVP 対象外**（単一 `armor` のみ。Phase 36 Counter System 以降で拡張可）。

**依存:** Phase 26 Boar の HP / 攻撃表示検証に本 Phase を先行させる。
