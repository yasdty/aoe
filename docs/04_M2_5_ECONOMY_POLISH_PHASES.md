...
- `BoarResource` + `BoarAggroManager` + `BoarAttackManager` — **HP / Food 分離**（Militia→HP、死体→Food）
- `AttackBoarCommand` / `HuntFoodCommand(BoarResource)` / Info Panel Food+Attack
- `AoE → Add Boars (Phase10)` — 既存シーンへ Boar + Manager 追加
...
- `Mill` / `MillRegistry` / `MillData` — Mining Camp パターン
- `FoodGatherManager.GetDepositPosition` — 最寄り TC / Mill
- HUD **Build Mill (100 Wood)**
...
**プロンプト:** [prompts/phase27-prompt.md](prompts/phase27-prompt.md)
...
| Deer / Sheep HP モデル統一 | Phase 28 optional（現状 Food 直減り = AoE2 簡略） |
| 本格 Pathfinding 回避 | M3 Phase 40 Formation / P3 |
| CPU 羊誘導 AI | **Phase 30** CPU 4 Resources と連携 |

**プロンプト:** [prompts/phase28-prompt.md](prompts/phase28-prompt.md)
...
**現状:** ~~Militia は右クリック攻撃のみ。~~ → `UnitAggroManager`（検知 5m / melee 2m / Console `[AutoAggro]`）
...
**プロンプト:** [prompts/phase29-prompt.md](prompts/phase29-prompt.md)

---

## Phase 30 — CPU 4 Resources ✅

**目的:** `CpuEconomyAiManager` が Wood / Food / Gold / Stone + 狩り / Farm / Camp を利用。**Economy / Military AI の競合を解消**し、Wood → Barracks → Militia 生産 → 全員攻撃の流れを維持。

**現状:** CPU は主に Wood 採集 + House + Villager。Berry / Farm / Gold / Stone 未対応。Mill 等が Barracks 用 Wood / builder を奪う可能性あり。

**実装方針:**

- 評価ループに Food / Gold / Stone 需要判断
- Villager 割当: Berry → 狩り → Farm / Gold / Stone / Wood
- **Barracks 完成前** — Economy Camp 建築を **延期** + Wood 予約（`CpuAiCoordination` 推奨）
- Mining Camp / Mill / Farm — Barracks 完成 & Wood 余裕時
- `CpuMilitaryAiManager` — **最小調整**（攻撃波 = 既存 Militia 全員、0 体ならスキップ。30 秒タイマーは維持）
- **Phase 28 依存:** CPU 羊発見・狩り（誘導 optional）
- **Command 化は不要**

**完了条件:**

- CPU が 4 資源を増やし Villager / Militia 生産を継続（資源不足時はスキップ、クラッシュなし）
- Barracks → Militia → 攻撃波の流れが Economy 拡張後も破綻しない

**プロンプト:** [prompts/phase30-prompt.md](prompts/phase30-prompt.md)
