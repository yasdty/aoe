...
> **前:** M2 Economy（Phase 17〜20） / **次:** [05_M3_MILITARY_PHASES.md](05_M3_MILITARY_PHASES.md)（Phase 29〜）
...
| Phase | 名称 | 目的 | 状態 |
|-------|------|------|------|
| 21 | Gather Repeat | 搬入後に同ノードへ自動復帰（採取リピート） | ⬜ 未着手 |
| 22 | Farm + Spawn | Farm 1 人制限 + 建物スポーン周囲グリッド | ⬜ 未着手 |
...
**現状:** 全 GatherManager が Deposit 後にジョブ削除で停止。

**変更対象:**

- `GatherManager`（Wood）
- `FoodGatherManager`（Berry + Farm）
- `MineralGatherManager`（Gold + Stone）

**完了条件:**

- 枯渇・新命令・死亡・Game Over でのみジョブ終了
- Wood / Food / Gold / Stone すべてでリピート動作
- Command Queue / Cancel 相互排他が維持

**プロンプト:** [prompts/phase21-prompt.md](prompts/phase21-prompt.md)（未作成）
...
**現状:** 複数村民が同一 Farm に `UnitPositionOffsets` で並ぶ。スポーンはカメラ方向出口 + 8 スロットリングのみ。

**実装方針:**

- `FoodGatherManager.IssueGatherFarmCommand` — 既に Gather 中の Farm は拒否 or 待機
- `TownCenter.GetVillagerSpawnPosition` / `Barracks.GetUnitSpawnPosition` — 建物外周グリッド（`GroupMoveFormation` 再利用可）
...
**プロンプト:** [prompts/phase22-prompt.md](prompts/phase22-prompt.md)（未作成）
...
**プロンプト:** [prompts/phase23-prompt.md](prompts/phase23-prompt.md)（未作成）
...
| 項目 | 理由 | 先送り先 |
|------|------|----------|
| 移動時ユニット押し出し / RVO | 性能・工数大 | M3 Phase 34 Formation / P3 Pathfinding |
| Stand Ground / Defensive スタンス UI | Phase 27 は簡易 Aggro のみ | M3 Phase 33 |
...
5. M2.5 全完了後 → [05_M3_MILITARY_PHASES.md](05_M3_MILITARY_PHASES.md) Phase 29 へ
