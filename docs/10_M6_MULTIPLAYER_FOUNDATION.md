# AoE RTS Engine — Milestone 6: Multiplayer Foundation ロードマップ（Phase 57〜61）
...
3. Entity ID（GameObject 非依存）   ⬜ Phase 57
4. 決定論シミュレーション           ⬜ Phase 59
5. CPU も Command 経由              ⬜ Phase 58
6. Command Log 保存 / 再生          ⬜ Phase 60
7. 複数 PlayerId / Team 拡張        ⬜ Phase 57
8. Network Transport（Lockstep）    ⬜ Phase 61 以降
9. View 分離（クライアント別 UI）   ⬜ Phase 52
```
...
| Phase | 名称 | 目的 | 状態 |
|-------|------|------|------|
| 57 | Entity ID & PlayerId | int Entity ID / PlayerId 4 チーム拡張 | ⬜ 未着手 |
| 58 | CPU Command Queue | CPU AI を `IGameCommand` 経由に統一 | ⬜ 未着手 |
| 59 | Deterministic Sim | Tick 順序固定・RNG シード・float 削減 | ⬜ 未着手 |
| 60 | Replay & Snapshot | Command Log ファイル I/O + 状態スナップショット | ⬜ 未着手 |
| 61 | Hotseat / Net Shell | ローカル 2 人交互 or ネットワーク層スタブ（送受信のみ） | ⬜ 未着手 |
